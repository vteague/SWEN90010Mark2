with Ada.Numerics;          use Ada.Numerics;
with Ada.Numerics.Float_Random;          use Ada.Numerics.Float_Random;
with Ada.Numerics.Elementary_Functions;  use Ada.Numerics.Elementary_Functions;
with Ada.Text_IO;

package body RandomFloat is

   -- a random number generator
   Seed  : Generator;

   -- Taken from: http://rosettacode.org/wiki/Random_numbers#Ada and
   -- corrected to use ln instead of log base 10
   function NormalDistribution(Mu    : Float;
                               Sigma : Float) return Float is
      Result : Float;
   begin
      Reset(Seed);
      Result := Mu + (Sigma * Sqrt (-2.0 * Log (Random (Seed))) * 
			Cos (2.0 * Pi * Random (Seed)));
      return Result;
   end NormalDistribution;
      
   function UniformDistribution(Lower, Upper : Float) return Float is
      Base : Uniformly_Distributed;
      Result : Float;
   begin
      Reset(Seed);
      Base := Random(Seed);
      Result := Lower + (Upper - Lower) * Base;
      return Result;
   end UniformDistribution;
   
   function UniformError(Value : in Float; Error : in Float) return Float is
      Lower, Upper : Float; -- The lower and upper bounds given the error
      RandomValue : Float;
   begin
      Reset(Seed);
      Lower := Value * (1.0 - Error);
      Upper := Value * (1.0 + Error);
      RandomValue := UniformDistribution(Lower, Upper);
      return RandomValue;
   end UniformError;
end RandomFloat;
